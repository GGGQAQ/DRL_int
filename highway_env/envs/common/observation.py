...
def observation_factory(env: 'AbstractEnv', config: dict) -> ObservationType:
    ...
    elif config["type"] == "GrayscaleKinematics":
        return GrayscaleKinematicObservation(env, **config)
    else:
        raise ValueError("Unknown observation type")


# class GrayscaleKinematicObservation(ObservationType):
#     '''
#     grayscale and kinematic!
#     '''
#
#     """
#     An observation class that collects directly what the simulator renders.
...
#
#     def space(self) -> spaces.Space:
#         return spaces.Box(shape=self.shape, low=0, high=255, dtype=np.uint8), spaces.Box(
#             shape=(self.vehicles_count, len(self.features)), low=-np.inf, high=np.inf, dtype=np.float32)
#
#     def normalize_obs(self, df: pd.DataFrame) -> pd.DataFrame:
#         """
#         Normalize the observation values.
...
#         return df
#
#     def kinematicObserve(self) -> np.ndarray:
#         if not self.env.road:
#             return np.zeros(self.space().shape)
#
#         # Add ego-vehicle
...
#         self.obs = np.roll(self.obs, -1, axis=0)
#         self.obs[-1, :, :] = new_obs
#         return self.obs, kinematicObserve()
#
#     def _render_to_grayscale(self) -> np.ndarray:
#         self.viewer.observer_vehicle = self.observer_vehicle
#         self.viewer.display()
